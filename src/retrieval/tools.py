...
class SearchLineItemsTool(BaseModel):
    ...
    min_amount: float | None = Field(
        None, description="Minimum line item total amount."
    )
    max_amount: float | None = Field(
        None, description="Maximum line item total amount."
    )
...
class SearchInvoicesTool(BaseModel):
    ...
    sender_name: str | None = Field(None, description="Filter by sender/vendor name.")
    invoice_number: str | None = Field(
        None, description="Exact invoice number to look up."
    )
    status: Literal["COMPLETED", "FAILED", "PROCESSING"] | None = Field(
        None, description="Filter by processing status."
    )
    filename_query: str | None = Field(
        None, description="Partial match for the filename."
    )

    start_date: str | None = Field(
        None, description="Filter by upload/invoice date (start). Format: YYYY-MM-DD"
    )
